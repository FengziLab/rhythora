...
<!-- Now Playing widget -->
<button onclick={toggleMusicPause} title={global.musicPlayerData.isPlaying === true ? "Pause" : "Play"} class="h-full px-4 rounded-lg hover:bg-zinc-500/50 active:translate-y-0.5 transition duration-100 ease-[ease] flex flex-row flex-nowrap gap-2 items-center justify-center">
    {#if global.musicPlayerData.isPlaying === true}
    ...
    {/if}
    <span class="text-zinc-50 text-lg font-comfortaa tracking-wide select-none">{displayText}</span>
</button>
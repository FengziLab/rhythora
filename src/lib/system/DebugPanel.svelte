<script lang="ts">
    ...
    import { audioContext, musicVolumeNode, setMusicVolume } from "$lib/system/audioContext";
...
<div class="absolute bottom-4 left-4 max-w-60 p-2 rounded-xl bg-zinc-800/90 border-t-2 border-b-2 border-zinc-800/90">
    ...
    <div class="flex flex-row flex-wrap gap-2">
        ...
        <button onclick={() => { setMusicVolume(2) }} class="h-8 px-2 rounded-md hover:bg-zinc-500/50 active:translate-y-0.5 transition duration-100 ease-[ease] flex items-center justify-center">
            <span class="text-zinc-100 text-xs font-mono select-none">Vol Up</span>
        </button>
        <button onclick={() => { setMusicVolume(1) }} class="h-8 px-2 rounded-md hover:bg-zinc-500/50 active:translate-y-0.5 transition duration-100 ease-[ease] flex items-center justify-center">
